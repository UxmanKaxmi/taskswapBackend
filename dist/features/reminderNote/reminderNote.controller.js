...
async function handleSendReminderNote(req, res, next) {
  try {
    ...
    if (!senderId) {
      return next(
        new errors_1.BadRequestError("User ID is missing from request.")
      );
    }
    const note = await (0, reminderNote_service_1.sendReminderNote)({
      taskId,
      senderId,
      message,
    });
...
async function handleGetRemindersByTask(req, res, next) {
  try {
    ...
    const notes = await (0, reminderNote_service_1.getRemindersByTask)(
      taskId,
      userId
    );
