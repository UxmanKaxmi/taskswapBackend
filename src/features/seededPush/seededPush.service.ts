...
import {
  createMotivationMilestoneNotification,
  createMotivationPushNotification,
...
async function getEligibleSeededPushPlan(taskId: string) {
  const config = getSeededPushConfig();

  if (!config.enabled || config.max <= 0) {
    return { task: null, selected: [], skipped: "disabled" as const };
  }
  ...
  const task = await prisma.task.findUnique({
    ...
    select: {
      ...
      Push: {
        where: { source: PUSH_SOURCE.SEEDED },
        select: { userId: true },
      },
    },
  });

  if (!task) return { task: null, selected: [], skipped: "missing-task" as const };
  if (task.type !== "motivation") return { task, selected: [], skipped: "non-motivation" as const };
  if (task.user.origin !== USER_ORIGIN.REAL) return { task, selected: [], skipped: "non-real-owner" as const };
  if (!task.isPublic || task.completed) return { task, selected: [], skipped: "inactive-task" as const };
  if (task.deliverAt && task.deliverAt.getTime() < Date.now()) {
    return { task, selected: [], skipped: "expired-task" as const };
  }
  ...
  if (existingSeededPusherIds.size >= config.max || task._count.Push >= config.max) {
    return { task, selected: [], skipped: "enough-pushes" as const };
  }

  const targetCount = randomInt(config.min, config.max);
  const availableSlots = Math.min(
    targetCount - existingSeededPusherIds.size,
    config.max - existingSeededPusherIds.size,
    config.max - task._count.Push
  );

  if (availableSlots <= 0) {
    return { task, selected: [], skipped: "no-slots" as const };
  }
...
async function createSeededPushForTaskUser({
  taskId,
  pusherId,
  maxSeededPushes,
}: {
  ...
  if (task.type !== "motivation") return false;
  if (task.user.origin !== USER_ORIGIN.REAL) return false;
  if (!task.isPublic || task.completed) return false;
  if (task.deliverAt && task.deliverAt.getTime() < Date.now()) return false;
  if (task.userId === pusherId) return false;
  ...
  const [existing, seededPushCount, totalPushCount] = await Promise.all([
    ...
    prisma.push.count({
      where: { taskId, source: PUSH_SOURCE.SEEDED },
    }),
    prisma.push.count({
      where: { taskId },
    }),
  ...
  if (seededPushCount >= maxSeededPushes || totalPushCount >= maxSeededPushes) return false;
  ...
  try {
    ...
    const pushCount = await prisma.push.count({ where: { taskId } });
...
export async function createSeededPushesForTask(taskId: string) {
  const config = getSeededPushConfig();
...
export async function scheduleSeededPushesForTask(taskId: string) {
  const config = getSeededPushConfig();
  const { selected, skipped } = await getEligibleSeededPushPlan(taskId);
  ...
  selected.forEach((candidate, index) => {
    const [minDelay, maxDelay] =
      seededPushDelayWindowsMs[index] ??
