...
export type CheerPresetKey = (typeof CHEER_PRESETS)[number]["key"];

export function getActiveCheerPreset(presetKey: string) {
  return CHEER_PRESETS.find(
    (preset) => preset.key === presetKey && preset.active
  );
}
