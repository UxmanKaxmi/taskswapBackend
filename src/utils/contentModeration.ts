...
export function assertPostableContent(
  ...
  if (finding) {
    throw new AppError(
      "This content cannot be posted. Please edit it and try again.",
...
function findObjectionableContent(text: string): ModerationFinding | null {
  if (!text) return null;

  const normalized = normalizeForModeration(text);
  const compact = normalized.replace(/\s+/g, "");

  const phraseMatch = BANNED_PHRASES.find((phrase) =>
  ...
  const abusiveWordMatch = ABUSIVE_WORDS.find((word) =>
  ...
  const spamPhraseCount = SPAM_PHRASES.filter((phrase) =>
    normalized.includes(phrase)
  ).length;
